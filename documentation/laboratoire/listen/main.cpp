...
int main() {
	...
	for (size_t i = 0; i < 5; i++)
	{
		...
		std::cout << CYAN << "Message reçu: " << NOCOLOR << buffer << std::endl;
		...
		close(client_fd);
		std::cout << GREEN << "Connexion client fermée" << NOCOLOR << std::endl;
	}

	// Fermer la socket serveur (ne sera jamais atteint dans cet exemple)
	close(server_fd);
